...
FROM 'C:\to path\Guide_Project_Data_Analysis.csv'
DELIMITER ','
CSV HEADER;
...
FROM store
/*ALL INFORMATIN IN SQL IS STORE IN INFORMATION_SCHEMA AND TWO METHOD USE ON THIS 
INFORMATION_SCHEMA.COLUMNS AND INFORMATION_SCHEMA.TABLES */

/* get dataset tables information*/
SELECT *
...
/* Which are the Top 10 Products that are ordered frequently? */
SELECT Product_Name, count(*) AS No_of_products,states,city
FROM store
GROUP BY Product_Name,states,city
order by  count(*) desc
limit 10
...
/* Where can we trim some loses? 
   ...
   can be analyzed deeper to see if its worth selling them an anymore. */
...
-- Find Top 10 Categories (with the addition of best sub-category within the category).:
SELECT  Category, Sub_Category ,city, round(cast(SUM(sales) as numeric),2) AS prod_sales
FROM store
GROUP BY Category,Sub_Category,city
ORDER BY prod_sales DESC;
...
--Check whether new customers are returning higher not.
SELECT Returned_items, Count(Returned_items)as Returned_Items_Count,Customer_duration
FROM store
GROUP BY Returned_items,Customer_duration
